...
Operator::Operator(yyltype loc, const std::string &text)
    : Node(loc),
      m_text(text)
{
}
...
PostfixExpr::PostfixExpr(Expr *left, Operator *op)
    : Expr(Join(left->GetLocation(), op->GetLocation())),
      m_left(left),
      m_op(op)
{
}

void PostfixExpr::PrintChildren(int indent_level)
{
    m_left->Print(indent_level + 1);
    m_op->Print(indent_level + 1);
}
